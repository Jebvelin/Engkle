...
Future maybeCreateUser(User user) async {
  final userRecord = PropertiesRecord.collection.doc(user.uid);
  final userExists = await userRecord.get().then((u) => u.exists);
  if (userExists) {
    currentUserDocument = await PropertiesRecord.getDocumentOnce(userRecord);
    return;
  }

  final userData = createPropertiesRecordData(
    email: user.email ??
  ...
  currentUserDocument =
      PropertiesRecord.getDocumentFromData(userData, userRecord);
}

Future updateUserDocument({String? email}) async {
  await currentUserDocument?.reference
      .update(createPropertiesRecordData(email: email));
}
